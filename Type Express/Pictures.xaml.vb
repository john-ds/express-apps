...
Public Class Pictures
    ...
    Private Sub Pictures_Loaded(sender As Object, e As RoutedEventArgs) Handles Me.Loaded
        ...
        Try
            ...
            ' ----------------------------------------------------
            Dim info = Windows.Application.GetResourceStream(New Uri("pack://application:,,,/Type Express;component/keypict.secret"))
    ...
    Private Function GetPics(query As String) As Boolean
        ...
        ' <urls>
        '     <raw>         --> give user choice of size to add to document
    ...
    Private Sub PhotoBtns_Click(sender As Button, e As RoutedEventArgs)
        ...
        If btnborder.BorderThickness.Top = 1 Then
            ...
            btnborder.BorderBrush = New SolidColorBrush(Color.FromRgb(60, 180, 60))
    ...
    Private Sub BtnSelected(id As String)
        TextSep.Visibility = Visibility.Visible
    ...
    Private Sub BtnUnselected()
        TextSep.Visibility = Visibility.Collapsed
            ...
            Try
                Using webc = New Net.WebClient()
                    Select Case ImageSize
                        Case "large"
                            Picture = New WinDrawing.Bitmap(New MemoryStream(webc.DownloadData(dict(SelectedPhoto)("raw") + "&w=600")))
                        Case "regular"
                            Picture = New WinDrawing.Bitmap(New MemoryStream(webc.DownloadData(dict(SelectedPhoto)("raw") + "&w=400")))
                        Case "small"
                            Picture = New WinDrawing.Bitmap(New MemoryStream(webc.DownloadData(dict(SelectedPhoto)("raw") + "&w=200")))
                        Case Else
                            Try
                                Picture = New WinDrawing.Bitmap(New MemoryStream(webc.DownloadData(dict(SelectedPhoto)("raw") + "&w=" + CustomTxt.Text)))
                            Catch
                                Picture = New WinDrawing.Bitmap(New MemoryStream(webc.DownloadData(dict(SelectedPhoto)("raw") + "&w=400")))
                            End Try
                    End Select
    ...
    Private Sub CustomTxt_TextChanged(sender As Object, e As RoutedEventArgs) Handles CustomTxt.LostFocus
        Try
            Dim customsize As Integer = Convert.ToInt32(CustomTxt.Text)
            If customsize < 50 Then
                CustomTxt.Text = "50"
            ElseIf customsize > 800 Then
                CustomTxt.Text = "800"
            End If
        Catch
            CustomTxt.Text = "400"
